...
//set date
        ...
        $('#genarate').click(function (e) {
            ...
            $.get("../../routes/invoice/getorderdata.php",{searchData:$inputData},function(res){
                var jdata = jQuery.parseJSON(res);

                if(jdata.order_payment=="1"){
                    $pay="success";
                }else{
                    $pay="waiting"
                }

                if(jdata.order_conform=="1"){
                    $conf="Conformed";
                }else{
                    $conf="waiting"
                }

                $('#oid').text(jdata.order_Id);
                $('#cid').text(jdata.user_Id);
                $('#odate').text(jdata.order_date);
                $('#oaddress').text(jdata.address);
                $('#ocon').text($conf);
                $('#opay').text($pay);
                $('#otot').text(jdata.order_price);
            })
         $.get("../../routes/invoice/getorderitems.php",{searchData:$inputData},function(res){
            $("#items").html(res);
         })
        })
